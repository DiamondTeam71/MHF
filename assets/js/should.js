...
// Load table with admin color support (hybrid matching)
function loadTable() {
  tableBody.innerHTML = '';
  ...
  // প্রথমে টেবিলের সর্বশেষ আপডেটেড ডেটা নিয়ে আসি
  const allRows = [];
  tableBody.querySelectorAll('tr').forEach(tr => {
    const tds = tr.querySelectorAll('td');
    const rowValues = [];
    // Actions কলাম বাদে বাকি ডেটা নাও
  ...
  firebase.database().ref(`users/${userId}/tables/${currentSector}`)
    ...
    .catch(err => alert('Delete error: ' + err.message));
          ...
          dup.onclick = () => {
            ...
            saveUserData({
              [currentSector]: { headers, rows: rows.map(r => r.values) }
            });
            loadTable();
          };
...
document.getElementById('exportCSV').addEventListener('click', () => {
  ...
  // Firebase public messages reference
  const publicRef = firebase.database().ref('public_messages');

  // Listen for updates
  publicRef.on('value', snap => {
    const data = snap.val() || {};
    document.getElementById('gmailPrize').textContent = data.gmailPrize || 'Gmail Prize দেখানো হবে এখানে';
    document.getElementById('adminMessage').textContent = data.adminMessage || 'Admin Message দেখানো হবে এখানে';
  });
